...
void TSDFVolumeCPU::map_ignore_test(std::vector<bool> dir_arr, float threshold){
    ...
    case direction::right:{
        ...
        for (; i < new_shift_range.end && j < old_keep_range.end; i++, j++){
            new_volume.at<Voxel>(0,i) = volume.at<Voxel>(0,j);
        }
        for (int k = new_create_range.start; k < new_create_range.end; k++){
            new_volume.at<Voxel>(0,k).weight = 0;
            new_volume.at<Voxel>(0,k).v = 0;
        }
    ...
    case direction::left:{
        int delete_old_start = length*length*(length-threshold);
        int delete_old_end = length*length*length;
        int keep_old_start =  0;
        int keep_old_end = delete_old_start;
        
        int create_new_start = 0;
        int create_new_end = length*length*threshold;
        int shift_new_start = create_new_end;
        int shift_new_end =  keep_old_end;
        ...
        std::cout<< "the map has been shifted in x_negative : left direction------------------------------" << std::endl;
        // std::cout <<"the size of the volume is " << volume.size() << std::endl;
        // std::cout <<"the size of the new_volume is " << new_volume.size() << std::endl;
        volume = new_volume; 
        break;
    ...
    raycastStepFactor(_raycastStepFactor)
{
    ...
    std::cout << "volResolution is " << volResolution << std::endl;//<<--------------------------------modification 
    std::cout << "voxelSize is " << voxelSize << std::endl;
    std::cout << "volSize is " << volSize << std::endl;
    ...
    // //--------------------------------------------------------------------modification
    // using namespace std;
    // cout << "the x direction size is "<< volResolution.x << endl;
    // //--------------------------------------------------------------------modification --didn't print out 
    if(zFirstMemOrder)
    {
...
{
    //std::cout << "hello world" << std::endl; //<<---------modification 
    CV_TRACE_FUNCTION();
...
struct IntegrateInvoker : ParallelLoopBody
{
    ...
    {   
        // //----------------------------------------------------------------------modification 
        // using namespace std;
        // cout << "the size of the range " << range.size() << endl;
        // //----------------------------------------------------------------------modification --the size() is not stable 
