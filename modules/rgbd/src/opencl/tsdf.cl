...
__kernel void integrate(__global const char* depthptr,
                        ...
                        __global const char* semanticptr,
                        int semantic_step, int semantic_offset,
                        int semantic_rows, int semantic_cols,
                        __global voxelType* volumeptr,
                        ...
                        const int maxWeight)
{
    ...
    for(int z = startZ; z < endZ; z++)
    {
        ...
        if(all(projected >= 0) && all(projected < limits))
        {
            ...
            float2 ip1 = round(projected);
            int xi1 = ip1.x, yi1 = ip1.y;
            __global const char* semantic_row = (__global const char*)(semanticptr + semantic_offset + yi1 * semantic_step);
        ...
        if(sdf >= -truncDist)
        {
            ...
            voxelType voxel = volumeptr[volIdx];
            float value = voxel.v;
            int weight = as_int(voxel.weight);
            float* semantic_weights = voxel.semantic_weights;


            // float3 updatedVoxel;
            // Copy the existing components
            // updatedVoxel.x = value;
            // updatedVoxel.y = as_float(weight);
